...
const FIELD_MAP = {
  ...
  // ── CAMPOS NUEVOS ────────────────────────────────────────────
  complejo:      ['complejo','complejo hospitalario','nombre del complejo',
                  'complejo sanitario','campus','edificio','nombre complejo'],
  province:      ['provincia','province','prov'],
  ccaa:          ['ccaa','comunidad autonoma','comunidad autónoma','comunidad',
                  'autonomia','región','region','c.a.','ca'],
  beds:          ['camas','beds','numero de camas','número de camas',
                  'n camas','num camas','nº camas','n.camas','plazas'],
...
async function _runImport() {
  ...
  const newRecords = iRows.map(row => {
    ...
    const r = {
      ...
      ccaa:          raw.ccaa      || null,
      complejo:      raw.complejo  || null,
      beds:          raw.beds ? (parseInt(String(raw.beds).replace(/\D/g,''), 10) || null) : null,
      phone:         raw.phone   || '',
      program:       raw.program || '',
      version:       raw.version || '',
      notes:         raw.notes   || '',
